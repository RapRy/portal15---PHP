...
<div class="py-4">
    <?php
        foreach($collections as $collection):
            ...
            $replaceHeader = str_replace(" ", "-", $collection["header"]);
    ?>
            <div class="pt-8">
                <div class="flex flex-row flex-wrap justify-between items-end mb-5">
                    <h1 class="font-bold text-sm text-textColor1 dark:text-neutralLight"><?php echo $collection["header"]; ?></h1>
                    <a class="text-xs font-medium text-textColor1 dark:text-neutralLight hover:text-primaryMain dark:hover:text-primaryMain transition-colors duration-300 ease-in-out" href="<?php echo "{$_SERVER['PHP_SELF']}?cat={$actives['catActive']}&sub={$replaceHeader}"; ?>" all="see more">see more (<?php echo count($collection["contents"]); ?>)</a>
                </div>
    ...
    ?>
                        <div class="bg-neutralLight dark:bg-textColor1 rounded-xl py-6 shadow-cardShadow <?php echo $ind === $lengthCollection ? "" : "mb-4"; ?> animate-pushUpMobile lg:animate-pushRightDesktop">
                            ...
                            <div class="px-6 pt-2">
                                <!-- link -->
                                <a href="<?php echo $content['filename']; ?>" class="rounded-md px-4 py-2 mr-2 hover:bg-neutralMain dark:hover:bg-textColor3 transition-colors duration-300 ease-in-out delay-75" alt="Play">
                                ...
                                <a href="#" class="rounded-md px-4 py-2 hover:bg-neutralMain dark:hover:bg-textColor3 transition-colors duration-300 ease-in-out delay-75" alt="Details">
